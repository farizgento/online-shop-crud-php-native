...
$data = mysqli_query($mysqli, "INSERT INTO produk SET nama='$nama', keterangan='$keterangan',harga='$harga', stok='$stok' ,kategori='$kategori',gambar='$name_p'") or die ("data salah : ".mysqli_error($mysqli));
if ($data) { 
...
<body class="bg">
  ...
  <div class="container border shadow-md p-5 mt-5 mb-5 bg-konten" style="background-color: white">
    <form action="" method="post" enctype="multipart/form-data">
        <div class="mb-3">
          <h3 class="fw-bold mt-2">Tambah Produk</h3>
        </div>
        <div class="mb-3">
            <label for="Nama" class="form-label">Nama Produk</label>
            <input type="text" class="form-control" id="nama" autocomplete="off" name="nama" required placeholder="masukan nama produk . . ." />
          </div>
        <div class= "mb-3">
          <label for="tgl" class="form-label">Keterangan</label>
          <input type="text" class="form-control" autocomplete="off" id="tgl" required name="keterangan" placeholder="masukan keterangan produk . . ." />
        </div>
      <div class= "mb-3">
        <label for="tl" class="form-label">Stok</label>
        <input type="number" class="form-control" id="tl" name="stok" autocomplete="off" required placeholder="masukan stok . . ." />
      </div>
      <div class="mb-3">
        <label for="jenis kelamin" class="form-label">Kategori</label>
        <div class="form-check">
          <input type="radio" class="form-check-input" id="elektronik" name="kategori" value="elektronik" />
          <label for="elektronik" class="form-check-label">Elektronik</label>
        </div>
        <div class="form-check">
          <input type="radio" class="form-check-input" id="peralatan_rumah" name="kategori" value="peralatan_rumah" />
          <label for="peralatan_rumah" class="form-check-label">Peralatan Rumah</label>
        </div>
        <div class="form-check">
          <input type="radio" class="form-check-input" id="otomotif" name="kategori" value="otomotif" />
          <label for="otomotif" class="form-check-label">Otomotif</label>
        </div>
      </div>
      <div class= "mb-5">
        <label for="harga" class="form-label">Harga</label>
        <input type="number" class="form-control" id="harga" autocomplete="off" name="harga" required placeholder="masukan harga produk . . ." />
      </div>
      <div class= "mb-5">
